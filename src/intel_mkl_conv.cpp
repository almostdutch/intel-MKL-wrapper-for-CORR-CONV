		...
		const int *const kernel_shape, const int *const data_in_shape, const int *const data_out_shape)
{
	...
	status = vslcConvNewTask(&task, mode, dims, kernel_shape, data_in_shape,
			data_out_shape);
	if (status != VSL_STATUS_OK) {
		cout << "ERROR: during the call vslcConvNewTask..." << endl;
		cout << "ERROR: exit with status: " << status << endl;
		exit(1);
	}
	
	status = vslConvSetInternalPrecision (&task, internal_precision);
	if (status != VSL_STATUS_OK) {
		cout << "ERROR: during the call vslConvSetInternalPrecision..." << endl;
		cout << "ERROR: exit with status: " << status << endl;
		exit(1);
	}
	
	status = vslcConvExec(task, kernel_MKL, NULL, data_in_MKL, NULL,
			data_out_MKL, NULL);
	if (status != VSL_STATUS_OK) {
		cout << "ERROR: during the call vslcConvExec..." << endl;
		cout << "ERROR: exit with status: " << status << endl;
		exit(1);
	}
	...
	if (status != VSL_STATUS_OK) {
		cout << "ERROR: during the call vslConvDeleteTask..." << endl;
		cout << "ERROR: exit with status: " << status << endl;
		exit(1);
	}
		...
		const int *const kernel_shape, const int *const data_in_shape, const int *const data_out_shape)
{
	...
	status = vslzConvNewTask(&task, mode, dims, kernel_shape, data_in_shape,
			data_out_shape);
	if (status != VSL_STATUS_OK) {
		cout << "ERROR: during the call vsldConvNewTask..." << endl;
		cout << "ERROR: exit with status: " << status << endl;
		exit(1);
	}
	...
	status = vslzConvExec(task, kernel_MKL, NULL, data_in_MKL, NULL,
			data_out_MKL, NULL);
	if (status != VSL_STATUS_OK) {
		cout << "ERROR: during the call vsldConvExec..." << endl;
		cout << "ERROR: exit with status: " << status << endl;
		exit(1);
	}
		...
		const int *const kernel_shape, const int *const data_in_shape, const int *const data_out_shape)
{
	...
	status = vslsConvNewTask(&task, mode, dims, kernel_shape, data_in_shape,
			data_out_shape);
	if (status != VSL_STATUS_OK) {
		cout << "ERROR: during the call vslcConvNewTask..." << endl;
		cout << "ERROR: exit with status: " << status << endl;
	...
	status = vslsConvExec(task, kernel, NULL, data_in, NULL,
			data_out, NULL);
	if (status != VSL_STATUS_OK) {
		cout << "ERROR: during the call vslcConvExec..." << endl;
		cout << "ERROR: exit with status: " << status << endl;
		...
		const int *const kernel_shape, const int *const data_in_shape, const int *const data_out_shape)
{
	...
	status = vsldConvNewTask(&task, mode, dims, kernel_shape, data_in_shape,
			data_out_shape);
	if (status != VSL_STATUS_OK) {
		cout << "ERROR: during the call vslcConvNewTask..." << endl;
		cout << "ERROR: exit with status: " << status << endl;
	...
	status = vsldConvExec(task, kernel, NULL, data_in, NULL,
			data_out, NULL);
	if (status != VSL_STATUS_OK) {
		cout << "ERROR: during the call vslcConvExec..." << endl;
		cout << "ERROR: exit with status: " << status << endl;
